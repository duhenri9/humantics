...
echo "📁 Transferindo arquivos para seu diretório local..."

# Verificar se está no diretório correto
if [ ! -d "bin" ]; then
    echo "❌ Você não está no diretório humantics-automation com o Docker Compose instalado"
    echo "Execute primeiro: cd humantics-automation"
    exit 1
fi

# Copiar docker-compose.yml do projeto
curl -o docker-compose.yml "https://raw.githubusercontent.com/humantics/raw/main/docker-compose.yml" 2>/dev/null || {
    echo "⚠️  Falha no download. Criando manualmente..."
...
echo ""
echo "🚀 Sistema: Docker + N8N + Venom-Bot + HumanTic Agent (com LGPD)!"